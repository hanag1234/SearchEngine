	...
	public static String fetch(URL url, int redirects) {
		...
		try {
			...
			if (getStatusCode(results) == 200 && isHtml(results)) {
				List<String> content = results.get("Content");
				html = String.join("\n", content);
			} else if (redirects > 0 && getStatusCode(results) >= 300 && getStatusCode(results) < 400) {
				String redirectUrl = getRedirect(results);
				if (redirectUrl != null) {
					html = fetch(new URL(redirectUrl), redirects - 1);
				}
			}
		} catch (IOException e) {
			html = null;
